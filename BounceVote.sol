...
contract BounceVote {
    ...
    struct Poll {
        ...
        string name;
        address creator;
        string[5] options; // Array to hold up to 5 options
        uint optionCount;  // Current count of options
        uint minQWLAHeld;
        uint voteCount;
        bool isOpen;
    ...
    function createPoll(string memory name, string[5] memory options, uint optionCount, uint minQWLAHeld) public onlyWhitelisted returns (uint) {
        require(optionCount >= 1 && optionCount <= 5, "Option count must be between 1 and 5");
    ...
    function addOption(uint pollId, string memory option) public {
        require(msg.sender == polls[pollId].creator, "Only the creator can add options");
        require(polls[pollId].optionCount < 5, "Cannot add more than 5 options");
        polls[pollId].options[polls[pollId].optionCount++] = option;
        emit OptionAdded(pollId, option);
    }
    ...
    function vote(uint pollId, uint option) public {
        ...
        require(option < polls[pollId].optionCount, "Invalid option");
